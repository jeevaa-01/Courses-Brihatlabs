...
test("certification status, identity, and issuance APIs require identity", async () => {
  for (const [path, method, headers] of [
    ["/api/courses/data-analytics/certification/status", "GET", {}],
    ["/api/courses/data-analytics/certification/identity", "POST", { "content-type": "application/json" }],
    ["/api/courses/data-analytics/certification/issue", "POST", { "Idempotency-Key": "test-key" }],
  ]) {
    const response = await render(path, headers, method);
    assert.equal(response.status, 401, path);
  }
});

test("certification page is protected and public verification page is lightweight", async () => {
  const protectedResponse = await render("/courses/data-analytics/certification");
  assert.equal(protectedResponse.status, 307);
  assert.match(protectedResponse.headers.get("location") || "", /signin-with-chatgpt/);
  const verifyResponse = await render("/verify/AGL-TEST-CREDENTIAL");
  assert.equal(verifyResponse.status, 200);
  assert.match(await verifyResponse.text(), /Certificate verification/);
});
